...
* **Intelligent Logic**: Google Gemini 3 Flash (Context-aware responses via `@google/generative-ai`).
...
│   ├── services/         # API abstraction and AI Service Logic
...
#### 1. Graceful Degradation & Offline Resilience
The platform is engineered to remain functional even under extreme conditions (API Quota Exhaustion or Network Loss).
* **Automated Fallback**: If the Gemini API returns a `429 (Quota Exceeded)` or `503` error, the system automatically switches to the **Local Knowledge Base**.
* **IndexedDB Persistence**: Utilizing Firebase’s offline persistence, the app queues analytics and interaction data while offline, syncing them instantly upon reconnection.
